...
fn create_sample_kv_csv() -> NamedTempFile {
    ...
    writeln!(file, "kv_path,unique_clients,operations_count,entity_ids,alias_names,sample_paths_accessed").unwrap();
...
fn test_kv_compare_command() {
    ...
    let result = kv_compare::run(
        csv1.path().to_str().unwrap(),
        csv2.path().to_str().unwrap()
    );
...
fn test_entity_timeline_command() {
    ...
    let result = entity_timeline::run(

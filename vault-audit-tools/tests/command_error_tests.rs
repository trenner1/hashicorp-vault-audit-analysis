...
fn test_kv_compare_with_temp_files() {
    ...
    writeln!(old_file, "path,entity_id,read_count,list_count,first_access,last_access").unwrap();
    
    let mut new_file = File::create(&new_path).unwrap();
    writeln!(new_file, "path,entity_id,read_count,list_count,first_access,last_access").unwrap();
...
fn test_system_overview_invalid_file() {
    ...
    let result = system_overview::run(&["/nonexistent/file.log".to_string()], 10, 1);
    assert!(result.is_err());
}
...
fn test_system_overview_empty_log() {
    ...
    let result = system_overview::run(&[log_path.to_str().unwrap().to_string()], 10, 1);
    assert!(result.is_ok());
}
